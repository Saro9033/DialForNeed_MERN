...
const Product = ({ isCategory, category, product, lg }) => {
    ...
    const truncateCategoryTitle = (title, maxLength) => {
        return title && title.length < maxLength ? title.slice(0, maxLength) + '...' : title;
    };

    const maxLength = windowWidth < 990 ? 15 : 20;
    const title = isCategory ? category?.title : product?.name;
    const displayedTitle = isCategory ? truncateCategoryTitle(title, 10) : truncateTitle(title, maxLength);
            ...
            <Card className="h-100 w-100 border-none shadow-sm" style={{ border: '0px', borderRadius: '5px' }}>
                <img
                    ...
                    style={{
                        ...
                        objectFit: isCategory ? 'contain' : 'cover',
                        fontFamily: 'Basis Grotesque Pro, sans-serif'
                    }}
                    height={windowWidth < 500 ? '110px' : windowWidth < 990 ? '150px' : '180px'}
                    width='100%'
                    src={isCategory ? category?.image : product?.images[0]?.image}
                    className="mx-auto"
                />
                <Card.Body className="px-3 d-flex flex-column pt-1 pb-3 mx-0">
                    <p className='mb-0'>
                        <Link
                            style={{
                                color: '#393F42',
                                textDecoration: 'none',
                                fontSize: windowWidth < 990 ? '15px' : '18px'
                            }}
                    ...
                    <div className="mt-auto">
                        ...
                        <Link style={{ textDecoration: 'none', color: 'unset' }} to={isCategory ? `/category-products/${category?._id}` : `/product/${product?._id}`}>
                            <Button style={{ fontSize: windowWidth < 990 ? '14px' : '15px', border: 'none', boxShadow: '0 0 0 0.2px black' }} className="view-btn py-1 btn-block">
                                {isCategory ? 'View Products' : 'View Details'}
                            </Button>
                        </Link>
                    </div>
                </Card.Body>
